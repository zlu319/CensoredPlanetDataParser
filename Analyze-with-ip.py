    ...
    for filename in filenames:
        with open(filename, 'r') as file:
            ...
            for line in file:
                ...
                try:
                    dns_responses = data_line['response'][0]['response'].keys()
                    # print(dns_responses)
                    ip = next(iter(dns_responses)) # choose the first ip
                    # print(ip)
                except:
                    if DEBUG:
                        print('ip extraction failure, set ip to', ip)
                ...
                if url not in website_stats:
                    website_stats[url] = [anom_ct, pass_ct, category, ip]
                else:
                    website_stats[url][0] += anom_ct
                    website_stats[url][1] += pass_ct
                    if website_stats[url][3] == '0.0.0.0':
                        website_stats[url][3] = ip
